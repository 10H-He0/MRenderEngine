...
bool TFbxLoader::LoadFBXMesh(std::wstring MeshName, MMesh& Mesh)
{
	//Get model path and convert to UTF8
	std::wstring ModelDir = MFileHelpers::EnginDir() + L"Resource/Models/";
...
bool TFbxLoader::ProcessNode(FbxNode* Node, MMesh& Mesh)
{
	if (Node->GetMesh())
	{
...
bool TFbxLoader::ImportMesh(FbxNode* Node, MMesh& Mesh)
{
	...
	for (uint32_t t = 0; t < numTriangles; t++)
	{
		for (uint32_t v = 0; v < 3; v++)
		{
			...
			//Add new index
			MMesh::uint32 index = MMesh::uint32(Mesh.Indices32.size());
